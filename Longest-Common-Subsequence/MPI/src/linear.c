...
void MPILinearMemLCS(mtype **scoreMatrix, int sizeA, int sizeB, char *seqA, char *seqB, int my_rank, int n_tasks){
  ...
  if (my_rank == 0){
    for (int j=1; j<=sizeA ;j++){
      ...
      nums[0] = scoreMatrix[i][j-1]; nums[1] = scoreMatrix[i][j];
      MPI_Send(nums, 2, MPI_SHORT, (my_rank+1)%n_tasks, STD_TAG, MPI_COMM_WORLD);
    }
  } else {
    for (int j=1; j<=sizeA ;j++){
      MPI_Recv(nums, 2, MPI_SHORT, (my_rank+n_tasks-1)%n_tasks, STD_TAG, MPI_COMM_WORLD, &status);
      if (seqA[j-1] == seqB[i*n_tasks+my_rank]) { scoreMatrix[i][j] = nums[0] + 1;
      } else { scoreMatrix[i][j] = max(scoreMatrix[i][j-1], nums[1]); }
      nums[0] = scoreMatrix[i][j-1]; nums[1] = scoreMatrix[i][j];
      MPI_Send(nums, 2, MPI_SHORT, (my_rank+1)%n_tasks, STD_TAG, MPI_COMM_WORLD);
    }
  }
  ...
  // Show result
  if (my_rank == ((sizeB-1) % n_tasks))
    printf("Rank: %d, Score: %d\n", my_rank, scoreMatrix[size-1][sizeA]);
}
